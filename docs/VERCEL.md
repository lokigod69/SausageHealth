...
| Loyverse | Catalogue and weekly sales connected 16 September, GET only, on request and once a day at 22:00 UTC. No webhook and no POS write. Stock is a dated snapshot, not live |
...
Runtime source `e373e07`; deployment `dpl_6oQ3ekHhjkjreEXtpxXwuPcVeB5L`, which added the confirmed 1-12 September renovation period to `SH_LOYVERSE_EXCLUDED_PERIODS`. Previously `37592e4` / `dpl_9SX9yiKS4wPxWu4pztsoFF5mcg7T`, https://sausage-health-hzaw78v1e-lokigod69s-projects.vercel.app . [CI run 35094759370](https://github.com/lokigod69/SausageHealth/actions/runs/35094759370) passed 126 backend tests (2 intentional skips), 12 Node tests, Docker build and container smoke. Earlier: `c211e1d` / `dpl_CubBdeYmBYPxe7JntPHA4XXEtGem`. Project crons are enabled and bound to this deployment: `/api/cron/loyverse` at `0 22 * * *`, verified through the project API rather than assumed. The endpoint was exercised against the live host: 404 with no secret, 404 with a wrong one, and a complete read with the real `CRON_SECRET`. Earlier: `1bd0baf` / `dpl_4JySoUECT4voLkDwdtiFmB3kBrWs`, https://sausage-health-ni65rvoll-lokigod69s-projects.vercel.app . [CI run 34999063654](https://github.com/lokigod69/SausageHealth/actions/runs/34999063654) passed the frontend build, **95 backend tests including the Postgres variants** (2 backend-specific skips), 11 Node tests, Docker build and container smoke. That run is the evidence for the new `loyverse_syncs` table on Postgres; the local machine has no Postgres and skips those cases.
