...
## 11 September 2026 · first real CSV intake

- Two supplied CSV originals preserved unchanged, moved into ignored private intake storage and saved in the authenticated live collection. A separate derived first-review report identifies source hashes/entry IDs, methods, coverage limits, unresolved cost/stock issues and next steps. No business figures or receipt/staff details are copied into this tracked evidence log.
- Six new synthetic analyzer tests passed locally: signed refunds/exact decimals, missing dates distinct from zero, duplicate keys, unexpected refund signs, non-finite values, arithmetic mismatch visibility, within-Handle shared fields, no cost/stock inheritance, conflicting/composite items and variable pricing. The actual data also passed an independent integer-cent net-sales and payment tie-out, catalogue counts and unchanged-source hash checks. Generic script only; no live POS/model was used.
- Authenticated uploads finalized three records. Repeated exact payloads returned the same entries, downloaded originals matched SHA-256, manager visibility passed and anonymous access was denied. All records remain awaiting human review. The owner account is the import actor; Moritz is recorded as source supplier.
- Desktop and 390×844 browser report rendering inspected. Mobile document width 375 within viewport 390, no horizontal overflow. Screenshots remain private in the batch folder. This does not complete a physical phone/camera/store-network test.
- Complete post-intake encrypted cloud backup saved outside Vercel/Neon. Authenticated decryption, expected record coverage, all three original-file hashes and session exclusion passed. The archive is not in Git. No new restore deployment was performed; the 9 September separate-target drill remains the live recovery evidence. No recurring backup worker is installed.
- Private provenance and exact evidence: `.data/intake/2026-09-11-loyverse/`. Current app runtime remains e89483d; this intake changes data and adds an admin analyzer/process docs, not the hosted application UI/API.
