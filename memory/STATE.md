...
**Published to https://ops.thesausageguy.shop on 16 September, and the live account has still not been read through this reader.** No `SH_LOYVERSE_*` variable is set in the Vercel production environment, so the page shows "Loyverse is not connected yet" to both accounts. It stays off until `SH_LOYVERSE_ENABLED=1` is set there; the credential in `.data/loyverse-access.json` alone does not switch it on. Switching it on, and setting `SH_LOYVERSE_STORE_MAP`, are owner actions. Until a store id is mapped, no Loyverse store is attributed to The Sausage Guy or Natural Mind Health, so the health-store question stays open rather than being answered by assuming the single returned store.
...
Desktop and 390×844 review/attachment UI inspected with no horizontal overflow; evidence is private. No physical-phone/camera/store-network result is claimed. Runtime is now **1bd0baf**, deployment **dpl_4JySoUECT4voLkDwdtiFmB3kBrWs**, at the ops alias, published on 16 September with the Items page; the previous runtime was **e89483d** / **dpl_8kFt6q4TwTfp7N3wdyHEuUM4XgUu**. Earlier hosted HTTPS/private access, 6 MiB upload/review/hash, redeployment persistence and real backup/restore passed.
